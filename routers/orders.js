...
router.post('/', async (req, res) => {
    ...
    if(!order){
        return res.status(404).send('The order could be created!');
    };
...
router.delete('/:id', (req, res) => {
    Order.findByIdAndRemove(req.params.id).then( async order => {
        if(order) {
            ...
            return res.status(404).json.apply({success: false, message: "order not found"})
        }
    }).catch( err => {
...
// getting the number of sales for the admin pannell
